module "mongodb" {
    source  = "terraform-aws-modules/ec2-instance/aws"   #Search in google aws terraform module
...
#Null resources ---paina instances lo emina changes vachinappudu trigger anedhi change avuthundhi. 
resource "null_resource" "mongodb" {
  ...
  #below connection thsruvatha remote exec call avuthundhi
  connection {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sh /tmp/bootstrap.sh mongodb dev" #provisioners lo Root access radhu by default so manam rasam sudo sh
    ]
  }
}
